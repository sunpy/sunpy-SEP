...
It should be noted that because `sunpy.timeseries` is using Pandas it will not
be changed to use `astropy.time` it would be necessary to migrate from Pandas
to `astropy.table` to gain the advantages of `astropy.time`. Timeseries data are
currently not sufficiently supported by `astropy.table` this is proposed to be
added in [APE 9](https://github.com/astropy/astropy-APEs/pull/12).
