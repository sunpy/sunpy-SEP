...
Applying this set of criteria on 2024/04/09 resulted in the following people being invited to be the initial Project Custodians:
...
* If at the end of the two week nomination period there are the same number of candidates as vacant seats on the committee, then the nomination period must be extended for one week. After which the candidate will be elected unopposed.
...
Any changes to the governance structure of the SunPy Project (e.g. this SEP) requires a 2/3 majority vote of the Active Project Custodians.
.
