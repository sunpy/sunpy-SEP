...
```python
>>> from sunpy.net.import SolarDownloader, attrs
>>> results = SolarDownloader.query(attrs.Time('2012/1/2 00:00:00','2012/1/2 00:10:00'),
                                    attrs.Instrument('AIA'), attrs.vso.Wave(304, 304),
                                    attrs.vso.Sample(60*10))
>>> results
[[(QueryResponseBlock)
{
...
The `query()` method of the `SolarDownloader` instance returns a `QueryResponse`
...
`SolarDownloader` class, this is a list of `Results` objects returned by each
...
```python
>>> from sunpy.net import SolarDownloader, attrs
>>> results = SolarDownloader.query(attrs.JSOC.series('hmi.m_45s'),attrs.Time('2012/2/2','2012/2/2'))
```
